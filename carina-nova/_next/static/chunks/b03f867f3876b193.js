...
}`}function n(e,t,s){return{deliverableType:t,subject:s,audience:"general",depth:"comprehensive",format:"structured HTML",constraints:[],referenceFileIds:[],suggestedTemplateId:e,rawUserIntent:`Use the "${t}" template.`}}function r(e,t,s){let a=t.map(e=>{let t=e.options?` Options: ${e.options.join(", ")}.`:"";return`- "${e.name}" (${e.type}): ${e.label} — ${e.description}${t}`}).join("\n");return`You are filling in template parameters for a "${e}" template based on a user request.
...
Example: { "subject": "GCP Professional Cloud Security Engineer", "audience": "experienced engineers" }`}e.s(["appendFileContext",()=>s,"briefToArchitectPrompt",0,a,"briefToChartPrompt",()=>a,"buildArchitectSystemPrompt",0,i,"buildChartSystemPrompt",()=>i,"buildCompassSystemPrompt",0,t,"buildNavSystemPrompt",()=>t,"buildParamInferencePrompt",()=>r,"buildTemplateBrief",()=>n])},43699,e=>{e.v(e=>Promise.resolve().then(()=>e(2904)))}]);